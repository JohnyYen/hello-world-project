...
The platform consists of three primary components and two shared packages, communicating primarily through a REST API contract:
...
graph TB
    ...
    subgraph API["API Layer"]
        APIClient[API Client Package<br/>packages/api-client-ts]
        OpenAPISpec[API Contract<br/>packages/api-contract<br/>openapi.json]
    end
    ...
    FB -->|HTTP/REST| APIClient
    GM -->|HTTP/REST| APIClient
    APIClient -->|Generated from| OpenAPISpec
    APIClient --> FastAPI
    GM -->|Direct HTTP<br/>(if no client)| FastAPI
    ...
    style API fill:#1a1b26,stroke:#7FB4CA
    style SERVER fill:#1a1b26,stroke:#A3BE8C
    style DATA fill:#1a1b26,stroke:#B48EAD
```

**Key communication patterns**:
- **Frontend ↔ Backend**: HTTP REST via auto-generated TypeScript client (`packages/api-client-ts`). Server Actions for mutations, direct fetch for reads.
...
### 7.2 API Contract Pattern

The API contract follows an OpenAPI-first approach:

1. **Source of Truth**: `packages/api-contract/openapi.json` — the authoritative specification of all API endpoints, request schemas, and response formats. Auto-generated by FastAPI (Swagger UI at `/docs`, ReDoc at `/redoc`).

2. **TypeScript Client Generation**: `packages/api-client-ts/` is auto-generated from the OpenAPI spec using OpenAPI Generator. Run `pnpm run generate:api-client` after backend changes to regenerate.

3. **Client Consumption**:
   - Frontend imports `@workspace/api-client-ts` for all API calls
   - Game uses direct HTTP calls (Godot's HTTPRequest node) for the same endpoints
   - Both consume identical request/response schemas
...
│       └── test/             # GUT test suite
├── packages/
│   ├── api-client-ts/        # Auto-generated TypeScript client from OpenAPI
│   └── api-contract/         # OpenAPI spec (source of truth for API)
├── infraestructure/
...
- **API Client** (`packages/api-client-ts/`): Versions ALWAYS match Backend MAJOR.MINOR for compatibility.
