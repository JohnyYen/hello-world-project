...
func execute(solution: SolutionData, context: BaseProblemContext) -> ExecutionContext:
	...
	if not first_block is StartBlock:
		print("Error: El primer bloque debe ser un StartBlock. Se encontro: ", first_block)
		is_valid = false

	if not last_block is EndBlock:
		print("Error: El ultimo bloque debe ser un EndBlock. Se encontro: ", last_block)
		is_valid = false
	...
	for block in blocks:
		...
		if block.has_method("execute"):
			...
			var error_msg = "Error: El bloque '" + block.name + "' no tiene metodo 'execute'"
			print(error_msg)
