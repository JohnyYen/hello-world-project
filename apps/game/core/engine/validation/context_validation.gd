...
func _generate_feedback(is_correct: bool, exec_ctx: ExecutionContext, problem_ctx: BaseProblemContext) -> String:
	if is_correct:
		return "¡Solución correcta! Has completado el objetivo."

	if exec_ctx.has_errors():
		return "Error durante la ejecución: " + exec_ctx.errors[0]

	return "La solución no resolvió el problema correctamente. Intenta de nuevo."
