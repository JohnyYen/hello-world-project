...
# =============================================================================
# track_custom with INTERACTED verb tests (Ciclo 3)
# =============================================================================

func test_track_custom_with_interacted_returns_statement() -> void:
	var stmt := service.track_custom(Verbs.INTERACTED, "game_action", "get_bread", "Tomar pan", "actor1")
	assert_eq(typeof(stmt), TYPE_DICTIONARY, "track_custom debe retornar un Dictionary")
	assert_false(stmt.is_empty(), "El statement no debe estar vacío")

func test_track_custom_interacted_has_correct_verb() -> void:
	var stmt := service.track_custom(Verbs.INTERACTED, "game_action", "get_bread", "Tomar pan", "actor1")
	assert_has(stmt, "verb_id", "El statement debe tener verb_id")
	assert_has(stmt.verb_id, "interacted", "El verb_id debe contener 'interacted'")
	assert_eq(stmt.verb_display, "interactuó", "El display debe ser 'interactuó'")

func test_track_custom_interacted_has_correct_object() -> void:
	var stmt := service.track_custom(Verbs.INTERACTED, "game_action", "attend_student", "Atender estudiante", "actor1")
	assert_eq(stmt.object_type, "game_action")
	assert_eq(stmt.object_id, "attend_student")
	assert_eq(stmt.object_name, "Atender estudiante")

func test_track_custom_interacted_has_actor() -> void:
	var stmt := service.track_custom(Verbs.INTERACTED, "game_action", "get_bread", "Tomar pan", "test_player_42")
	assert_eq(stmt.actor_id, "test_player_42")

func test_track_custom_interacted_is_persisted() -> void:
	service.track_custom(Verbs.INTERACTED, "game_action", "serve_bread", "Servir pan", "actor1")
	var pending := service.get_pending_statements(50)
	var found := false
	for s in pending:
		if s.object_id == "serve_bread" and s.verb_display == "interactuó":
			found = true
			break
